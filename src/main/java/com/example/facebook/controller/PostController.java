...
public class PostController extends AbstractController {
    ...
    @PostMapping("/posts")
    public ResponseEntity<NewsFeedDTO> createPost(@RequestBody CreatePostDTO dto,
                                                  @RequestParam long pageNumber,
                                                  @RequestParam long rowsNumber,
                                                  HttpSession session){
        long userID = getUserByID(session);
        return new ResponseEntity<>(postService.createPost(userID, dto, pageNumber, rowsNumber), HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<NewsFeedDTO> editPost(@RequestBody EditPostDTO dto,
                                                ...
                                                HttpSession session){
        long userID = getUserByID(session);
        return new ResponseEntity<>(postService.editPost(userID, postID, dto, pageNumber, rowsNumber),
                                                                                                    HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<NewsFeedDTO> reactToPostOrDislike(@PathVariable(name = "pid") long postID,
                                                            ...
                                                            HttpSession session){
        long userID = getUserByID(session);
        return new ResponseEntity<>(postService.reactToPostOrDislike(userID, postID, reaction, pageNumber, rowsNumber),
                                                                                                         HttpStatus.OK);
    }
}
